...
static uint32_t _fs_search_last_log (fs_folder_def *fsfolder, uint32_t log_req);
static uint8_t  _T_H_log_decode (uint8_t *buffer, uint32_t address, uint32_t length);
...
/**
  ...
  * @retval None
  */
static  uint32_t  _fs_search_last_log			(fs_folder_def *fsfolder, uint32_t log_num)
{
    ...
    // searching for the tags (start and end)
		while(buf_idx < MX25R6435F_SECTOR_SIZE){
          ...
          terminal(" - Log Nr:%d   buf_idx:%.4d   address: 0x%X \n", count, buf_idx, address);
            ...
            status = _T_H_log_decode(QSPI_Buffer, fsfolder->LastLogAddr, fsfolder->LogLength + 4);	// +4 because i dont count tags here
            return status;          
          }
			...
			if(tag_idx == 2) ++fsfolder->LogLength;
...
uint8_t   _fs_log (char *data, fs_folder_def *fsfolder)
{
  ...
  // check for the end of spi flash memory
	...
	// check for the end of spi flash memory
	if (address>= fsfolder->EndAddr){
		terminal("\nEnd Address is reached, Folder is full!\n");
		return ERROR;
	}
...
/**
  ...
  * @retval None
  */
void	T_H_struct_init (fs_folder_def *fsfolder)
{
...
//  strcpy(fsfolder->Commant, "the folder used for stroing the temperature and Humidity logs");
	...
	terminal("\nLog Count: %d\nLast log : 0x%X", _fs_search_last_log(fsfolder, 0), fsfolder->LastLogAddr);
}
/**
  * @brief  
  * @param  None
...
uint8_t  T_H_log_finder (fs_folder_def *fsfolder, uint32_t log_num)
{
  ...
  if(_fs_search_last_log(fsfolder, log_num) != SUCCESS){
		return ERROR;
	}
  return SUCCESS;
